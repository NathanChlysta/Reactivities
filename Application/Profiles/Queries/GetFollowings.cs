...
public class GetFollowings
{
    ...
    public class Handler(AppDbContext context, IMapper mapper, IUserAccesor userAccesor) : IRequestHandler<Query, Result<List<UserProfile>>>
    {
            ...
            switch (request.Predicate)
            {
                case "followers":
                    profiles = await context.UserFollowings.Where(x => x.TargetId == request.UserId)
                        .Select(x => x.Observer)
                        .ProjectTo<UserProfile>(mapper.ConfigurationProvider, new {currentUserId = userAccesor.GetUserId()})
