...
public class GetActivityList
{
    ...
    public class Handler(AppDbContext context, IMapper mapper, IUserAccesor userAccesor) : IRequestHandler<Query, Result<PagedList<ActivityDto, DateTime?>>>
    {
        public async Task<Result<PagedList<ActivityDto, DateTime?>>> Handle(Query request, CancellationToken cancellationToken)
        {
            ...
            if (!string.IsNullOrEmpty(request.Params.Filter))
            {
                ...
                {
                    "isGoing" => query.Where(x => x.Attendees.Any(a => a.UserId == userAccesor.GetUserId())),
                    "isHost" => query.Where(x => x.Attendees.Any(a => a.IsHost && a.UserId == userAccesor.GetUserId())),
                    _ => query
                };
            }

            var projectedActivities = query.ProjectTo<ActivityDto>(mapper.ConfigurationProvider,
                    new { currentUserId = userAccesor.GetUserId() });
